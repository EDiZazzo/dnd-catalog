...
function tableToMarkdown($table, $) {
  ...
  $table.find('tr').each((i, tr) => {
    ...
    $(tr).find('th, td').each((j, cell) => {
      cells.push($(cell).text().trim().replace(/\s+/g, ' ').replace(/\|/g, '\\|'));
    });
  ...
  if (actualHeaders.length === 0 && actualRows.length > 0) {
    actualHeaders = actualRows[0];
    actualRows = actualRows.slice(1);
  }

  if (actualHeaders.length > 0) {
    markdown += '| ' + actualHeaders.join(' | ') + ' |\n';
      ...
      contentDiv.children().each((idx, el) => {
        ...
        if (tag === 'h1' || tag === 'h2' || tag === 'h3' || tag === 'h4' || tag === 'h5') {
          ...
          if (lvlMatch) {
            ...
            const shouldTerminate = /^(Subclasses|.*Subclass Options|Multiclassing|Creating a.*)$/i.test(text);
            if (shouldTerminate) {
              features.push(currentFeature);
              currentFeature = null;
            } else {
          ...
          if (!firstHeadingHit) {
            ...
            if (tag === 'p') {
              ...
              if (tableMd.trim().length > 0) {
                currentFeature.description_paragraphs.push(tableMd);
              }
            }
          }
        }
            ...
            if (tag === 'h1' || tag === 'h2' || tag === 'h3' || tag === 'h4' || tag === 'h5') {
              ...
              if (!firstSubHeadingHit) {
                ...
                if (tag === 'p') {
                  ...
                  const tableMd = tableToMarkdown($el, $sub);
                  if (tableMd.trim().length > 0) {
                    currentSubFeature.description_paragraphs.push(tableMd);
                  }
                }
              }
            }
...
async function main() {
  ...
  try {
    // 1. Load spells from cache instead of scraping
    const officialSpells = JSON.parse(fs.readFileSync(path.join(outputDir, 'official_spells.json'), 'utf8'));
    ...
    // 2. Load UA spells from cache instead of scraping
    const uaSpells = JSON.parse(fs.readFileSync(path.join(outputDir, 'ua_spells.json'), 'utf8'));
